...
""" Parse a man page in mdoc format, and optionally check that all
    options are included in a completion file or edit a bash completion file.
"""
...
def parse_cmdline():
    ...
    parser.add_argument("--edit-bash",
                        help="edit the bash completion file")
    args = parser.parse_args()

    # Sanity check.
    if "mdoc" not in args.filename_manpage:
        raise Exception("man-page must be in mdoc format")
    if args.check_file and args.edit_bash:
        assert args.check_file != args.edit_bash
...
def main(args):
    ...
    if args.edit_bash:
        man_to_completion.bash_completion_edit(args.edit_bash, options)
